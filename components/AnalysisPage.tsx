...
import { Sparkles, Brain, AlertCircle, RefreshCw, FileText, ChevronRight, Star, Clock, Target, TrendingUp, Activity, Radio, Orbit } from 'lucide-react';
...
const errors = [
  {
    id: 1,
    type: 'Grammar',
    title: 'Relativity Syntax Error',
    source: 'Cambridge 15 - Test 3 - Writing Task 2',
    content: 'The number of students who studies abroad has increased.',
    correction: 'The number of students who study abroad has increased.',
    insight: '在 "who" 引导的定语从句中，谓语动词应与先行词 "students" (复数) 保持一致，而非 "number"。这是一个典型的高频失分点。'
  },
  {
    id: 2,
    type: 'Logic',
    title: 'Hyperspace Inference Failure',
    source: 'Cambridge 16 - Test 1 - Reading Passage 2',
    content: '原文未提及未来计划，却选择了 "Future Plan" 选项。',
    correction: 'Answer: Not Given',
    insight: '雅思阅读中，"Not Given" 意味着文中找不到依据，而非根据常识推断。你倾向于脑补文中未提及的信息。'
  },
  {
    id: 3,
    type: 'Vocabulary',
    title: 'Signal Match Lost (Synonyms)',
    source: 'Listening Section 3',
    content: '未能识别 "hazardous" 与 "dangerous" 的对应。',
    correction: 'Keyword match: hazardous = dangerous',
    insight: '词汇广度不足。Hazardous 常用于描述环境或健康风险，建议建立 "Risk" 主题词汇库。'
  }
];
...
const AnalysisPage: React.FC<AnalysisPageProps> = ({ isDarkMode }) => {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      ...
      setVariantQuestion(`[Lazarus Simulation - 语法强化]

请判断以下句子是否正确，若错误请改正：
"The group of teachers who leads the research project are arriving tomorrow."

(TARS 提示：注意集合名词 group 和定语从句中的主谓一致关系)`);
    }, 2000);
  };
                  ...
                  <div className={`h-full backdrop-blur-2xl border rounded-[32px] p-8 shadow-2xl relative overflow-hidden flex flex-col ${isDarkMode ? 'bg-black/40 border-white/10' : 'bg-white/40 border-white/50'}`}>
                      ...
                      <div className="relative z-10 flex justify-between items-start mb-8 border-b border-gray-200/10 pb-6">
                          <div>
                              ...
                              <h2 className="text-3xl font-bold inherit">{selectedError.title}</h2>
