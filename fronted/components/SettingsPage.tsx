  ...
  return (
    <main className="flex-1 min-h-screen pl-0 md:pl-24 pr-0 md:pr-8 py-8 md:py-10 max-w-[1600px] mx-auto z-10 relative overflow-hidden">
      ...
      <header className="mb-12 px-6 md:px-0 relative z-10">
        <h1 className="text-4xl font-bold tracking-tight mb-2 inherit">控制中心</h1>
      ...
      <div className="grid grid-cols-12 gap-8 px-6 md:px-0 relative z-10">
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4">
          <div className={`p-2 rounded-[24px] border ${settings?.isDarkMode ? 'bg-white/5 border-white/10' : 'glass-panel border-white/60'}`}>
            <SettingsNavItem icon={<Cpu size={20} />} label="通用设置" active isDarkMode={settings?.isDarkMode} />
            <SettingsNavItem icon={<Key size={20} />} label="API 配置" isDarkMode={settings?.isDarkMode} />
            <SettingsNavItem icon={<Shield size={20} />} label="隐私与安全" isDarkMode={settings?.isDarkMode} />
            <SettingsNavItem icon={<Volume2 size={20} />} label="声音与触感" isDarkMode={settings?.isDarkMode} />
          </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                      <span className="text-[10px] font-bold bg-gray-100/10 text-gray-500 px-2 py-1 rounded-md border border-gray-200/20">已加密</span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100/10">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-60">状态: {deepSeekKey ? '已就绪' : '未配置'}</span>
                  </div>
                  <button
                    onClick={handleSave}
